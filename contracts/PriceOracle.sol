...
contract PriceOracle is Exponential {
    ...
    function setExchangeRate(
        ...
        require(
            maxSwingDuration > 0 && maxSwingDuration <= SECONDS_PER_WEEK,
            "setExchangeRate: maxSwingDuration cannot be zero, less than 604800 (seconds per week)."
        );

        uint256 currentExchangeRate =
            ExchangeRateModel(exchangeRateModel).getExchangeRate();
        require(
        ...
        uint256 maxSwingRate =
            ExchangeRateModel(exchangeRateModel).getMaxSwingRate(
                maxSwingDuration
            );
        require(
            maxSwingRate > 0 &&
                maxSwingRate <=
                ExchangeRateModel(exchangeRateModel).getMaxSwingRate(SECONDS_PER_WEEK),
        ...
        returns (uint256)
    {
        ...
        uint256 newMaxSwingRate =
            exchangeRateModel.getMaxSwingRate(maxSwingDuration);
        ...
        require(
            newMaxSwingRate > 0 &&
                newMaxSwingRate <= exchangeRateModel.getMaxSwingRate(SECONDS_PER_WEEK),
    ...
    function assetPrices(address asset) public view returns (uint256) {
        ...
        if (paused) {
            ...
            if (exchangeRateInfo.exchangeRateModel != address(0)) {
                uint256 scale =
                    ExchangeRateModel(exchangeRateInfo.exchangeRateModel)
                        .scale();
                uint256 currentExchangeRate =
                    ExchangeRateModel(exchangeRateInfo.exchangeRateModel)
                        .getExchangeRate();
        ...
        returns (uint256)
    {
        ...
        if (localVars.pendingAnchorMantissa != 0) {
            ...
            if (localVars.anchorPeriod != 0) {
                ...
                localVars.anchorPrice = Exp({
                    mantissa: requestedPriceMantissa
                });
            }
        }
