...
contract RewardDistributor is Initializable, Ownable, IRewardDistributor {
        ...
        override
    {
        ...
        for (uint256 i = 0; i < _iTokens.length; i++) {
            ...
            _updateDistributionState(address(_iToken), false);
            _updateDistributionState(address(_iToken), true);
            for (uint256 j = 0; j < _holders.length; j++) {
                _updateReward(address(_iToken), _holders[j], false);
                _updateReward(address(_iToken), _holders[j], true);
            }
        }
     ...
     * @param _holders The account to claim for
     */
    function claimAllReward(address[] memory _holders) public override {
