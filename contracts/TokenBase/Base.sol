...
abstract contract Base is TokenERC20 {
    ...
    function _flashloanInternal(
        ...
        uint256 _cashBefore = _getCurrentCash();
        ...
        // Transfers funds to caller contract, it will fail if `_loanAmount` > `_getCurrentCash()`.
        ...
        if (protocolFeeRatio > 0) {
            // Protocol fee is the part of the flashloan fee.
            _protocolFee = _flashloanFee.rmul(protocolFeeRatio);

            // Updates cash and reserves according to flashloan fee.
            totalReserves = _protocolFee.add(totalReserves);
        }

        controller.afterFlashloan(address(this), _recipient, _loanAmount);
