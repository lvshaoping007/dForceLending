...
describe("iETH", async function () {
  ...
  async function init() {
    ...
    await iToken.connect(borrower).mint(borrower.address, actualiTokenMintAmount);
    await iToken.connect(liquidator).mint(liquidator.address, actualiTokenMintAmount);
    await iToken1.connect(minter).mint(minter.address, actualiToken1MintAmount);
    await iToken1.connect(borrower).mint(borrower.address, actualiToken1MintAmount);
    await iToken1.connect(liquidator).mint(liquidator.address, actualiToken1MintAmount);
  }
  ...
  function calcBorrowBalance(borrowIndex, borrowData) {
    ...
    if (borrowData[0].eq(zero) || borrowData[1].eq(zero))
        return zero;

    return borrowData[0].mul(borrowIndex).div(borrowData[1]);
  }

  // Calculate the collateral that can be liquidated with iToken
  async function calcSeizeTokens(borrowiToken, collateralAddress, collateralExchangeRate, repayAmount) {

    let liquidationIncentive = await controller.liquidationIncentiveMantissa();
    let valueRepayPlusIncentive = repayAmount.mul(await oracle.getUnderlyingPrice(borrowiToken.address)).mul(liquidationIncentive).div(BASE);

    return valueRepayPlusIncentive.mul(BASE).div(collateralExchangeRate).div(await oracle.getUnderlyingPrice(collateralAddress));
  }
  ...
  describe("Test all scenarios for Mint", async function () {
    it("Initialize data and check basic minting data", async function () {
      ...
      ).to.changeEtherBalances([minter, iETH], [mintAmount.mul(-1), mintAmount]);

      expect(data.cash.add(mintAmount)).to.equal(await iETH.getCash());
      expect(data.totalSupply.add(mintToken)).to.equal(await iETH.totalSupply());
      expect(beforeBalanceOf.add(mintToken)).to.equal(await iETH.balanceOf(minter.address));
    });
    ...
    it("Should mint for another account", async function () {
      ...
      ).to.changeEtherBalances([minter, mintAnother, iETH], [mintAmount.mul(-1), 0, mintAmount]);

      expect(data.cash.add(mintAmount)).to.equal(await iETH.getCash());
      expect(data.totalSupply.add(mintToken)).to.equal(await iETH.totalSupply());
      expect(beforeBalanceOfAccount).to.equal(await iETH.balanceOf(minter.address));
      expect(beforeBalanceOfAnother.add(mintToken)).to.equal(await iETH.balanceOf(mintAnother.address));
    });
    ...
    it("When the supply interest rate is greater than 0, minting operations", async function () {
      ...
      ).to.changeEtherBalances([iETH, borrower], [borrowAmount.mul(-1), borrowAmount]);

      // check iETH data
      expect(data.cash.sub(borrowAmount)).to.equal(await iETH.getCash());
      expect(data.totalBorrows.add(borrowAmount)).to.equal(await iETH.totalBorrows());
      ...
      expect(borrowBalance.add(borrowAmount)).to.equal(await iETH.borrowBalanceStored(borrower.address));
      expect((await iETH.borrowSnapshot(borrower.address))[0]).to.equal(await iETH.borrowBalanceStored(borrower.address));
      ...
      let expectedUnderlying = expectedIncrementDLETH.add(beforeDLETHBalance).mul(data.exchangeRate).div(BASE);
      expect(beforeDLETHBalance.add(expectedIncrementDLETH)).to.equal(await iETH.balanceOf(minter.address));
      expect(expectedUnderlying).to.equal(await iETH.callStatic.balanceOfUnderlying(minter.address));
    });
    ...
    it("When the user supply is zero, the user will not get DLETH and will not transfer ETH", async function () {
      ...
      let expectedUnderlying = beforeDLETHBalance.mul(data.exchangeRate).div(BASE);
    ...
    it("Should revert due to user do not have enough ETH", async function () {
      try {
        await iETH.connect(minter).mint(minter.address, {value: await minter.getBalance()})
      } catch (error) {
        // console.log(error);
      }
    });
    ...
    it("Should revert due to user address is 0 address", async function () {
      await expect(
        iETH.connect(minter).mint(ethers.constants.AddressZero, {value: ethers.utils.parseEther("1")})
    ...
    it("Should revert due to controller refuse to mint", async function () {
      ...
      await expect(
        iETH.connect(minter).mint(minter.address, {value: ethers.utils.parseEther("1")})
      ...
      await controller._setSupplyCapacity(iETH.address, data.exchangeRate.mul(data.totalSupply).div(BASE).add(ethers.utils.parseUnits("1", "wei")));
      await expect(
        iETH.connect(minter).mint(minter.address, {value: ethers.utils.parseUnits("2", "wei")})
  ...
  describe("Test all scenarios for Redeem", async function () {
    it("Should redeem from userself normally", async function () {
      ...
      ).to.changeEtherBalances([redeemer, iETH], [mintAmount.mul(-1), mintAmount]);

      expect(data.cash.add(mintAmount)).to.equal(await iETH.getCash());
      expect(data.totalSupply.add(mintToken)).to.equal(await iETH.totalSupply());
      expect(beforeBalanceOf.add(mintToken)).to.equal(await iETH.balanceOf(redeemer.address));

      // redeem.
      data = await getiTokenCurrentData(iETH, 1);
      beforeBalanceOf = await iETH.balanceOf(redeemer.address);
      let beforeBalanceOfUnderlying = beforeBalanceOf.mul(data.exchangeRate).div(BASE);

      let redeemAmount = ethers.utils.parseEther("12.5987");
      // Estimated amount of ETH redeemed
      let redeemUnderlyingAmount = redeemAmount.mul(data.exchangeRate).div(BASE);
      // Redeem and check the amount of ETH
      await expect(() =>
        iETH.connect(redeemer).redeem(redeemer.address, redeemAmount)
      ).to.changeEtherBalances([redeemer, iETH], [redeemUnderlyingAmount, redeemUnderlyingAmount.mul(-1)]);

      expect(data.cash.sub(redeemUnderlyingAmount)).to.equal(await iETH.getCash());
      expect(data.totalSupply.sub(redeemAmount)).to.equal(await iETH.totalSupply());
      expect(beforeBalanceOf.sub(redeemAmount)).to.equal(await iETH.balanceOf(redeemer.address));
      expect(beforeBalanceOfUnderlying.sub(redeemUnderlyingAmount)).to.gte(await iETH.callStatic.balanceOfUnderlying(redeemer.address));
      expect(data.exchangeRate).to.lte(await iETH.callStatic.exchangeRateCurrent());

      // redeemUnderlying.
      data = await getiTokenCurrentData(iETH, 1);
      beforeBalanceOf = await iETH.balanceOf(redeemer.address);
      beforeBalanceOfUnderlying = beforeBalanceOf.mul(data.exchangeRate).div(BASE);

      redeemUnderlyingAmount = ethers.utils.parseEther("1.598729151");
      
      // Estimated amount of iETH burned
      redeemAmount = redeemUnderlyingAmount.mul(BASE).add(data.exchangeRate.sub(ethers.utils.parseUnits("1", "wei"))).div(data.exchangeRate);

      // redeemUnderlying and check the amount of ETH
      await expect(() =>
        iETH.connect(redeemer).redeemUnderlying(redeemer.address, redeemUnderlyingAmount)
      ).to.changeEtherBalances([redeemer, iETH], [redeemUnderlyingAmount, redeemUnderlyingAmount.mul(-1)]);
    ...
    it("Should redeem from another", async function () {
      ...
      let allowance = await iETH.allowance(redeemer.address, mintAnother.address);

      // Redeem and check the amount of ETH
      await expect(() =>
        iETH.connect(mintAnother).redeem(redeemer.address, redeemAmount)
      ).to.changeEtherBalances([mintAnother, redeemer, iETH], [redeemUnderlyingAmount, 0, redeemUnderlyingAmount.mul(-1)]);

      expect(data.cash.sub(redeemUnderlyingAmount)).to.equal(await iETH.getCash());
      expect(data.totalSupply.sub(redeemAmount)).to.equal(await iETH.totalSupply());
      expect(beforeBalanceOf.sub(redeemAmount)).to.equal(await iETH.balanceOf(redeemer.address));
      expect(beforeBalanceOfUnderlying.sub(redeemUnderlyingAmount)).to.equal(await iETH.callStatic.balanceOfUnderlying(redeemer.address));
      expect(data.exchangeRate).to.lte(await iETH.callStatic.exchangeRateCurrent());
      expect(allowance.sub(redeemAmount)).to.equal(await iETH.allowance(mintAnother.address, mintAnother.address));
      ...
      // redeemUnderlying and check the amount of ETH
      await expect(() =>
        iETH.connect(mintAnother).redeemUnderlying(redeemer.address, redeemUnderlyingAmount)
      ).to.changeEtherBalances([mintAnother, redeemer, iETH], [redeemUnderlyingAmount, 0, redeemUnderlyingAmount.mul(-1)]);
      ...
      await expect(
        iETH.connect(mintAnother).redeem(redeemer.address, ethers.utils.parseUnits("1", "wei"))
      ).to.be.revertedWith("SafeMath: subtraction overflow");

      // approved amount is insufficient, redeemUnderlying failed
      await expect(
        iETH.connect(mintAnother).redeemUnderlying(redeemer.address, ethers.utils.parseUnits("1", "wei"))
      ).to.be.revertedWith("SafeMath: subtraction overflow");
    });
    ...
    it("Redeem should revert due to insufficient balance", async function () {
      ...
      let beforeBalanceOf = await iETH.balanceOf(redeemer.address);
      await expect(
        iETH.connect(redeemer).redeem(redeemer.address, beforeBalanceOf.add(ethers.utils.parseUnits("1", "wei")))
      ).to.be.revertedWith("SafeMath: subtraction overflow");

      let data = await getiTokenCurrentData(iETH, 1);
      let beforeBalanceOfUnderlying = beforeBalanceOf.mul(data.exchangeRate).div(BASE);
      await expect(
        iETH.connect(redeemer).redeemUnderlying(redeemer.address, beforeBalanceOfUnderlying.add(ethers.utils.parseUnits("1", "wei")))
      ).to.be.revertedWith("SafeMath: subtraction overflow");
    });
    ...
    it("Redeem should revert due to redeem 0", async function () {
      ...
      await expect(
        iETH.connect(redeemer).redeem(redeemer.address, ethers.utils.parseUnits("0", "wei"))
      ).to.be.revertedWith("_redeemInternal: Redeem iToken amount should be greater than zero!");

      await expect(
        iETH.connect(redeemer).redeemUnderlying(redeemer.address, ethers.utils.parseUnits("0", "wei"))
      ).to.be.revertedWith("_redeemInternal: Redeem iToken amount should be greater than zero!");
    });
    ...
    it("Redeem should revert due to insufficient cash", async function () {
      ...
      await iETH.connect(minter).redeem(minter.address, await iETH.balanceOf(minter.address));
      ...
      let beforeBalanceOfUnderlying = beforeBalanceOf.mul(data.exchangeRate).div(BASE);
      // The user's ETH is greater than cash
      expect(beforeBalanceOfUnderlying).to.gt(cash.add(ethers.utils.parseUnits("1", "wei")));

      // Redeem underlying failed due to insufficient cash
      await expect(
        iETH.connect(redeemer).redeemUnderlying(redeemer.address, cash.add(ethers.utils.parseUnits("1", "wei")))
      ).to.be.revertedWith("reverted: function call failed to execute");
    });
    ...
    it("Should revert due to controller can not verify", async function () {
      ...
      await iToken.connect(redeemer).mint(redeemer.address, actualiTokenMintAmount);
      ...
      let redeemerEquityInfo = await controller.calcAccountEquity(redeemer.address);
      let borrowMaxAmount = redeemerEquityInfo[0].mul(marketInfo[1]).div(BASE).div(await oracle.getUnderlyingPrice(iToken1.address));

      // redeemer loan the maximum iToken
      await expect(() =>
        iToken1.connect(redeemer).borrow(borrowMaxAmount)
      ).to.changeTokenBalances(underlying1, [iToken1, redeemer], [borrowMaxAmount.mul(-1), borrowMaxAmount]);
      ...
      // iETH no collateral
      expect(await controller.hasEnteredMarket(redeemer.address, iETH.address)).to.equal(false);
      ...
      expect(await controller.hasEnteredMarket(redeemer.address, iETH.address)).to.equal(true);

      // When iETH is used as collateral, lend out all the net value
      redeemerEquityInfo = await controller.calcAccountEquity(redeemer.address);
      let borrowAmount = redeemerEquityInfo[0].mul(marketInfo[1]).div(BASE).div(await oracle.getUnderlyingPrice(iToken1.address)).sub(ethers.utils.parseUnits("1", "mwei"));
      expect(borrowAmount).to.gt(0);

      await expect(() =>
        iToken1.connect(redeemer).borrow(borrowAmount)
      ).to.changeTokenBalances(underlying1, [iToken1, redeemer], [borrowAmount.mul(-1), borrowAmount]);

      // euqity is 0, redeem failed
      await expect(
        iETH.connect(redeemer).redeem(redeemer.address, await iETH.balanceOf(redeemer.address))
      ).to.be.revertedWith("Account has some shortfall");

      // euqity is 0, redeemUnderlying failed
      redeemUnderlyingAmount = await iETH.callStatic.balanceOfUnderlying(redeemer.address);
      await expect(
        iETH.connect(redeemer).redeemUnderlying(redeemer.address, redeemUnderlyingAmount)
      ).to.be.revertedWith("Account has some shortfall");
      ...
      ).to.changeTokenBalances(underlying1, [iToken1, redeemer], [borrowBalance, borrowBalance.mul(-1)]);

      // After the loan of iToken1 is repaid, iToken1 will no longer be used as collateral
      expect(await controller.hasBorrowed(redeemer.address, iToken1.address)).to.equal(false);
  ...
  describe("Test all scenarios for Borrow", async function () {
    it("Should borrow normally", async function () {
      ...
      let borrowBalance = calcBorrowBalance(data.borrowIndex, borrowSnapshot);
      expect(borrowBalance.add(borrowAmount)).to.equal(await iETH.borrowBalanceStored(borrower.address));
      expect((await iETH.borrowSnapshot(borrower.address))[0]).to.equal(await iETH.callStatic.borrowBalanceCurrent(borrower.address));
    });
    ...
    it("Borrow should revert due to insufficient cash", async function () {
      await expect(
        iETH.connect(borrower).borrow((await iETH.getCash()).add(ethers.utils.parseUnits("1", "wei")))
      ).to.be.revertedWith("reverted: function call failed to execute");
    });
    ...
    it("Should revert due to controller refuse to borrow", async function () {
      ...
      let redeemerEquityInfo = await controller.calcAccountEquity(borrower.address);
      let borrowAmount = redeemerEquityInfo[0].mul(marketInfo[1]).div(BASE).div(await oracle.getUnderlyingPrice(iETH.address));
  ...
  describe("Test all scenarios for repay", async function () {
    it("Should repay for user self normally!", async function () {
      ...
      ).to.changeEtherBalances([borrower, iETH], [repayAmount.mul(-1), repayAmount]);

      expect(data.cash.add(repayAmount)).to.equal(await iETH.getCash());
      expect(data.totalBorrows.sub(repayAmount)).to.equal(await iETH.callStatic.totalBorrowsCurrent());
      expect(data.totalReserves).to.equal(await iETH.totalReserves());
      expect(data.borrowRate).to.gte(await iETH.borrowRatePerBlock());

      let borrowBalance = calcBorrowBalance(data.borrowIndex, borrowSnapshot);
      expect(borrowBalance.sub(repayAmount)).to.equal(await iETH.borrowBalanceStored(borrower.address));
      expect((await iETH.borrowSnapshot(borrower.address))[0]).to.equal(await iETH.borrowBalanceStored(borrower.address));
      expect(await controller.hasBorrowed(borrower.address, iETH.address)).to.equal(true);
    });
    ...
    it("When there is a loan, repay 0", async function () {
      ...
      expect(data.cash.add(repayAmount)).to.equal(await iETH.getCash());
      expect(data.totalBorrows.sub(repayAmount)).to.equal(await iETH.totalBorrows());
      expect(data.totalReserves).to.equal(await iETH.totalReserves());
      expect(data.borrowRate).to.lte(await iETH.borrowRatePerBlock());
    ...
    it("Should pay off all loans!", async function () {
      ...
      await expect(() =>
        iETH.connect(borrower).repayBorrow({value: borrowBalance.add(ethers.utils.parseEther("1"))})
      ).to.changeEtherBalances([borrower, iETH], [borrowBalance.mul(-1), borrowBalance]);

      expect(data.cash.add(borrowBalance)).to.equal(await iETH.getCash());
      expect(data.totalBorrows.sub(borrowBalance)).to.equal(await iETH.totalBorrows());
      expect(data.totalReserves).to.equal(await iETH.totalReserves());
      expect(data.borrowRate).to.gte(await iETH.borrowRatePerBlock());
      expect(borrowBalance.sub(borrowBalance)).to.equal(await iETH.borrowBalanceStored(borrower.address));
      expect((await iETH.borrowSnapshot(borrower.address))[0]).to.equal(await iETH.borrowBalanceStored(borrower.address));
      expect(await controller.hasBorrowed(borrower.address, iETH.address)).to.equal(false);
  ...
  describe("Test all scenarios for repay behalf", async function () {
    ...
    it("Should repay behalf for others normally!", async function () {
      ...
      let borrowAmount = (await iETH.getCash()).div(ethers.utils.parseUnits("2", "wei"));
      await expect(() =>
        iETH.connect(borrower).borrow(borrowAmount)
      ).to.changeEtherBalances([iETH, borrower], [borrowAmount.mul(-1), borrowAmount]);
      ...
      let data = await getiTokenCurrentData(iETH, 1);
      let borrowSnapshot = await iETH.borrowSnapshot(borrower.address);
      let anotherborrowSnapshot = await iETH.borrowSnapshot(mintAnother.address);
      let repayAmount = ethers.utils.parseEther("3.1415926");

      // repayBorrowBehalf 3.1415926 ETH and and check the amount of ETH
      await expect(() =>
        iETH.connect(mintAnother).repayBorrowBehalf(borrower.address ,{value: repayAmount})
      ).to.changeEtherBalances([mintAnother, borrower, iETH], [repayAmount.mul(-1), 0, repayAmount]);
      ...
      let anotherBorrowBalance = calcBorrowBalance(data.borrowIndex, anotherborrowSnapshot);
      expect(anotherBorrowBalance).to.equal(await iETH.borrowBalanceStored(mintAnother.address));
      expect((await iETH.borrowSnapshot(mintAnother.address))[0]).to.equal(await iETH.borrowBalanceStored(mintAnother.address));
      expect(await controller.hasBorrowed(mintAnother.address, iETH.address)).to.equal(false);
    ...
    it("Should pay off all loans!", async function () {
      ...
      // When the repayment amount is greater than the loan amount, only the loan amount is charged to repay the loan
      await expect(() =>
        iETH.connect(mintAnother).repayBorrowBehalf(borrower.address, {value: borrowBalance.add(ethers.utils.parseEther("1"))})
      ).to.changeEtherBalances([mintAnother, borrower, iETH], [borrowBalance.mul(-1), 0, borrowBalance]);
  ...
  describe("Test all scenarios for liquidate borrow", async function () {
    it("Should liquidate normally", async function () {
      ...
      await iToken.connect(borrower).redeem(borrower.address, iTokenBalance.sub(iTokenBalance.div(ethers.utils.parseUnits("20", "wei"))));
      await iToken1.connect(borrower).redeem(borrower.address, iToken1Balance.sub(iToken1Balance.div(ethers.utils.parseUnits("20", "wei"))));

      let marketInfo = await controller.markets(iETH.address);
      let borrowerEquityInfo = await controller.calcAccountEquity(borrower.address);
      let borrowMaxAmount = borrowerEquityInfo[0].mul(marketInfo[1]).div(BASE).div(await oracle.getUnderlyingPrice(iETH.address));

      // Borrow to the maximum, euqity is 0
      await expect(() =>
        iETH.connect(borrower).borrow(borrowMaxAmount)
      ).to.changeEtherBalances([iETH, borrower], [borrowMaxAmount.mul(-1), borrowMaxAmount]);

      //Update interest, shortfall greater than 0
      await iETH.connect(borrower).updateInterest();
      expect((await controller.calcAccountEquity(borrower.address))[1]).to.gt(0);
      ...
      await expect(() =>
        iETH.connect(liquidator).liquidateBorrow(borrower.address, iToken.address, {value: repayAmount})
      ).to.changeEtherBalances([liquidator, iETH, borrower], [repayAmount.mul(-1), repayAmount, 0]);
      ...
      expect(borrowBalance.sub(repayAmount)).to.equal(await iETH.callStatic.borrowBalanceCurrent(borrower.address));

      // Check the amount of iToken obtained through liquidation
      let seizeToken = await calcSeizeTokens(iETH, iToken.address, await iToken.exchangeRateStored(), repayAmount);
      expect(liquidatoriTokenBalance.add(seizeToken)).to.equal(await iToken.balanceOf(liquidator.address));
    });
    ...
    it("Should revert due to do not allow to liquidate self", async function () {
      ...
      await expect(
        iETH.connect(borrower).liquidateBorrow(borrower.address, iToken.address, {value: ethers.utils.parseUnits("10", "wei")})
      ).to.be.revertedWith("_liquidateBorrowInternal: Liquidator can not be borrower!");
    });
    ...
    it("Should revert due to do not allow to repay 0", async function () {
      ...
      ).to.be.revertedWith("_liquidateBorrowInternal: Liquidate amount should be greater than 0!");
    });
    ...
    it("Should revert due to controller refuses to liquidate: repay exceeds max repay allowed", async function () {
      ...
      let marketInfo = await controller.markets(iETH.address);
      let borrowerEquityInfo = await controller.calcAccountEquity(borrower.address);
      let borrowMaxAmount = borrowerEquityInfo[0].mul(marketInfo[1]).div(BASE).div(await oracle.getUnderlyingPrice(iETH.address)).sub(ethers.utils.parseUnits('1.8', 'szabo'));
      await expect(() =>
        iETH.connect(borrower).borrow(borrowMaxAmount)
      ).to.changeEtherBalances([iETH, borrower], [borrowMaxAmount.mul(-1), borrowMaxAmount]);
      ...
      let borrowSnapshot = await iETH.borrowSnapshot(borrower.address);
      let borrowBalance = calcBorrowBalance(data.borrowIndex, borrowSnapshot);
      let repayAmount = borrowBalance.mul(await controller.closeFactorMantissa()).div(BASE).add(ethers.utils.parseUnits("1", "wei"));
      await expect(
        iETH.connect(liquidator).liquidateBorrow(borrower.address, iToken.address, {value: repayAmount})
    ...
    it("Should revert due to controller refuses to liquidate: liquidation can be exchanged for insufficient collateral", async function () {
      ...
      let seizeToken = await calcSeizeTokens(iETH, iToken.address, (await getiTokenCurrentData(iToken, 1)).exchangeRate, repayAmount);
      ...
      seizeToken = await calcSeizeTokens(iETH, iETH.address, data.exchangeRate, repayAmount);
      expect(seizeToken).to.gt(await iETH.balanceOf(borrower.address));

      await expect(
        iETH.connect(liquidator).liquidateBorrow(borrower.address, iETH.address, {value: repayAmount})
      ).to.be.revertedWith("SafeMath: subtraction overflow");
    });
    ...
    it("Liquidation when borrowed assets and collateral are not the same", async function () {
      ...
      await expect(() =>
        iETH.connect(liquidator).liquidateBorrow(borrower.address, iToken.address, {value: repayAmount})
      ).to.changeEtherBalances([liquidator, borrower, iETH], [repayAmount.mul(-1), 0, repayAmount]);
      ...
      expect(borrowBalance.sub(repayAmount)).to.equal(await iETH.callStatic.borrowBalanceCurrent(borrower.address));
      expect(borroweriTokenBalance.sub(seizeToken)).to.equal(await iToken.balanceOf(borrower.address));
      expect(liquidatoriTokenBalance.add(seizeToken)).to.equal(await iToken.balanceOf(liquidator.address));
    });
    ...
    it("Liquidation when the borrowed assets are the same as the collateral", async function () {
      ...
      let marketInfo = await controller.markets(iETH.address);
      let borrowerEquityInfo = await controller.calcAccountEquity(borrower.address);
      let borrowMaxAmount = borrowerEquityInfo[0].mul(marketInfo[1]).div(BASE).div(await oracle.getUnderlyingPrice(iETH.address)).sub(ethers.utils.parseUnits('1', 'szabo'));
      await expect(() =>
        iETH.connect(borrower).borrow(borrowMaxAmount)
      ).to.changeEtherBalances([iETH, borrower], [borrowMaxAmount.mul(-1), borrowMaxAmount]);
      ...
      ).to.changeEtherBalances([borrower, iETH], [mintAmount.mul(-1), mintAmount]);
      ...
      let repayAmount = ethers.utils.parseEther("2");
      let seizeToken = await calcSeizeTokens(iETH, iETH.address, data.exchangeRate, repayAmount);
      ...
      expect(borrowBalance.sub(repayAmount)).to.equal(await iETH.callStatic.borrowBalanceCurrent(borrower.address));
      expect(borroweriTokenBalance.sub(seizeToken)).to.equal(await iETH.balanceOf(borrower.address));
      expect(liquidatoriTokenBalance.add(seizeToken)).to.equal(await iETH.balanceOf(liquidator.address));
    });
  ...
  describe("Test all scenarios for seize", async function () {
    ...
    it("Should revert due to liquidator can not be borrower", async function () {
      await expect(
        iETH.connect(borrower).seize(borrower.address, borrower.address, ethers.utils.parseUnits("10", "wei"))
    ...
    it("Should revert due to controller refuses to seize: Tokens have not been listed", async function () {
      await expect(
        iETH.connect(borrower).seize(liquidator.address, borrower.address, ethers.utils.parseUnits("10", "wei"))
  ...
  describe("Test all scenarios for Flashloan", async function () {
    it("Generate a flashloan as expected", async function () {
      let flashloanAmount = await iETH.getCash();
      let flashloanFeeAmount = (await iETH.flashloanFeeRatio()).mul(flashloanAmount).div(BASE);
      // add enough ETH to the flashloan executor contract to repay.
      await minter.sendTransaction({to: flashloanExecutor.address, value: flashloanFeeAmount});

      let data = await getiTokenCurrentData(iETH, 1);
      // execute flashloan.
      await expect(() =>
        iETH.connect(minter).flashloan(flashloanExecutor.address, flashloanAmount, "0x")
      ).to.changeEtherBalances([minter, flashloanExecutor, iETH], [0, flashloanFeeAmount.mul(-1),  flashloanFeeAmount]);
      ...
      expect(data.totalSupply).to.equal(await iETH.totalSupply());
      expect(data.totalReserves.add(flashloanFeeAmount.mul(await iETH.protocolFeeRatio()).div(BASE))).to.equal(await iETH.totalReserves());
    ...
    it("Should revert due to borrow too much cash", async function () {
      // execute flashloan.
      await expect(
        iETH.connect(minter).flashloan(flashloanExecutor.address, (await iETH.getCash()).add(ethers.utils.parseUnits("1", "wei")), "0x")
      ).to.be.revertedWith("reverted: function call failed to execute");      
    });
    ...
    it("Should revert due to borrow too small", async function () {
      // only borrow 1 wei.
      await expect(
        iETH.connect(minter).flashloan(flashloanExecutor.address, ethers.utils.parseUnits("1", "wei"), "0x")
      ).to.be.revertedWith("_flashloanInternal: Request amount is too small for a flashloan!");

      // only borrow 0 wei.
      await expect(
        iETH.connect(minter).flashloan(flashloanExecutor.address, ethers.utils.parseUnits("0", "wei"), "0x")
      ).to.be.revertedWith("_flashloanInternal: Request amount is too small for a flashloan!");
    });

    it("Should revert due to do not repay enough", async function () {
      await expect(
        iETH.connect(minter).flashloan(flashloanExecutorFailure.address, await iETH.getCash(), "0x")
      ).to.be.revertedWith("_flashloanInternal: Fail to repay borrow with fee!");
    });

    it("Should revert due to controller refuses", async function () {
      await controller._setBorrowPaused(iETH.address, true);
      await expect(
        iETH.connect(minter).flashloan(flashloanExecutor.address, await iETH.getCash(), "0x")
      ).to.be.revertedWith("Token borrow has been paused");
      await controller._setBorrowPaused(iETH.address, false);
    });
  ...
  describe("Test all scenarios for ERC20", async function () {
    ...
    it("Should revert due to user transfer to self", async function () {
      // execute transfer.
      await expect(
        iETH.connect(minter).transfer(minter.address, await iETH.balanceOf(minter.address))
    ...
    it("Should revert due to user transfer to 0 address", async function () {
      // execute transfer.
      await expect(
        iETH.connect(minter).transfer(ethers.constants.AddressZero, await iETH.balanceOf(minter.address))
    ...
    it("Test transferFrom", async function () {
      ...
      await expect(() =>
        iETH.connect(redeemer).transferFrom(minter.address, redeemer.address, amount)
      ).to.changeTokenBalances(iETH, [minter, redeemer], [amount.mul(-1), amount]);
      expect(data.cash).to.equal(await iETH.getCash());
      expect(data.totalBorrows).to.equal(await iETH.totalBorrows());
      expect(data.totalSupply).to.equal(await iETH.totalSupply());
      expect(data.totalReserves).to.equal(await iETH.totalReserves());
    ...
    it("Test decreaseAllowance", async function () {
      ...
      expect(allowance.sub(amount)).to.equal(await iETH.allowance(minter.address, redeemer.address));
    });

    it("Should revert due to user approve 0 address", async function () {
      await expect(
        iETH.connect(minter).approve(ethers.constants.AddressZero, await iETH.balanceOf(minter.address))
  ...
  describe("Test all scenarios for updateInterest", async function () {
    ...
    it("Should revert due to user transfer to self", async function () {
      ...
      await iETH.connect(owner)._setInterestRateModel(mockInterestRateModel.address)

      // execute updateInterest.
      await expect(
        iETH.connect(minter).updateInterest()
      ).to.be.revertedWith("_updateInterest: Borrow rate is too high!");
    });
  });
});
